			...
			localCapabilities.codecs.forEach(function(codec) {
				...
				if (transceiver.remoteCapabilities &&
					...
					transceiver.remoteCapabilities.codecs.forEach(function(remoteCodec) {
						if (codec.name.toLowerCase() === remoteCodec.name
							.toLowerCase() &&
...
class MediaStreamFactory {
	...
	static createMediaStream(constraints) {
		...
		if (!isVideoConstrainsForScreenCast(constraints) && typeof constraints.audio === 'object' && constraints
			.audio.source === AudioSourceInfo.SCREENCAST) {
		...
		if (isVideoConstrainsForScreenCast(constraints) && typeof constraints.audio === 'object' && constraints
			.audio.source !== AudioSourceInfo.SCREENCAST) {
			return Promise.reject(new TypeError('Cannot capture video from screen cast while capture audio from' +
				' other source.'));
		...
		if (typeof constraints.video === 'object') {
			...
			if (constraints.video.resolution && constraints.video.resolution.width && constraints.video.resolution
				.height) {
...
var settle = function settle(resolve, reject, response) {
	...
	if (!response.status || !validateStatus || validateStatus(response.status)) {
		...
		reject(new AxiosError_1(
			...
			[AxiosError_1.ERR_BAD_REQUEST, AxiosError_1.ERR_BAD_RESPONSE][Math.floor(response.status /
				100) - 4],
			response.config,
			response.request,
			response
...
var xhr = function xhrAdapter(config) {
	return new Promise(function dispatchXhrRequest(resolve, reject) {
		...
		function onloadend() {
			...
			var responseHeaders = 'getAllResponseHeaders' in request ? parseHeaders(request
		...
		if ('onloadend' in request) {
			...
			request.onreadystatechange = function handleLoad() {
				...
				if (request.status === 0 && !(request.responseURL && request.responseURL.indexOf(
						'file:') === 0)) {
					return;
				}
		...
		request.ontimeout = function handleTimeout() {
			var timeoutErrorMessage = config.timeout ? 'timeout of ' + config.timeout + 'ms exceeded' :
				'timeout exceeded';
			...
			reject(new AxiosError_1(
				timeoutErrorMessage,
				transitional$1.clarifyTimeoutError ? AxiosError_1.ETIMEDOUT : AxiosError_1
				.ECONNABORTED,
				config,
				request));
		...
		if (protocol && ['http', 'https', 'file'].indexOf(protocol) === -1) {
			reject(new AxiosError_1('Unsupported protocol ' + protocol + ':', AxiosError_1.ERR_BAD_REQUEST,
				config));
			return;
		}
...
var defaults = {
	...
	transformRequest: [function transformRequest(data, headers) {
		...
		if ((isFileList = utils.isFileList(data)) || (isObjectPayload && contentType ===
				'multipart/form-data')) {
		...
		if (strictJSONParsing || (forcedJSONParsing && utils.isString(data) && data.length)) {
			try {
				...
				if (strictJSONParsing) {
					if (e.name === 'SyntaxError') {
						throw AxiosError_1.from(e, AxiosError_1.ERR_BAD_RESPONSE, this, null, this
							.response);
					}
					throw e;
				}
...
class RTCEndpoint extends Event$1 {
	...
	start() {
		...
		if (this.options.useCamera) {
			...
			// @@@@@@@@@@@@@@@@@ 选择的麦克风id
			if (this.options.micId != '' && typeof videoConstraints == 'object') {
				audioConstraints.deviceId = this.options.micId;
			}
		} else {
			if (this.options.videoEnable) {
				videoConstraints = new VideoTrackConstraints(VideoSourceInfo.SCREENCAST);
				if (this.options.audioEnable) audioConstraints = new AudioTrackConstraints(AudioSourceInfo
					.SCREENCAST);
			} else {
		...
		if (this.options.resolution.w != 0 && this.options.resolution.h != 0 && typeof videoConstraints ==
			'object') {
			videoConstraints.resolution = new Resolution(this.options.resolution.w, this.options.resolution.h);
		}

		// @@@@@@@@@@@@@@@@@ 选择的摄像头id
		if (this.options.deviceId != '' && typeof videoConstraints == 'object') {
			videoConstraints.deviceId = this.options.deviceId;
		}
		// @@@@@@@@@@@@@@ 设置弹出共享框默认标签页
		if (this.options.displaySurface != '' && typeof videoConstraints == 'object') {
			videoConstraints.displaySurface = this.options.displaySurface;
		}

		MediaStreamFactory.createMediaStream(new StreamConstraints(audioConstraints, videoConstraints)).then(
			stream => {
				...
				this.pc.createOffer().then(desc => {
					...
					this.pc.setLocalDescription(desc).then(() => {
						axios({
							...
							if (ret.code != 0) {
								// mean failed for offer/anwser exchange 
								this.dispatch(Events$1
									.WEBRTC_OFFER_ANWSER_EXCHANGE_FAILED, ret);
								return;
							}
...
function GetSupportCameraResolutions$1() {
	return new Promise(function(resolve, reject) {
		...
		for (let i = 0; i < quickScan.length; ++i) {
			...
			MediaStreamFactory.createMediaStream(new StreamConstraints(false, videoConstraints)).then(
				stream => {
